...
private:
	...
	ID3D11ShaderResourceView* especularMap;
...
public:
	ModeloEspecular(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, const char NomOBJ[], WCHAR* diffuseTex, WCHAR* normal, WCHAR* especular, float esc, float rx, float ry, float x, float y, float z)
	{
		...
		CargaParametros(NomOBJ, diffuseTex, normal, especular);
	...
	bool CargaParametros(const char NomOBJ[], WCHAR* diffuseTex, WCHAR* normal, WCHAR* especular)
	{
		HRESULT d3dResult;
		...
		bool compileResult = CompileD3DShader(L"ModeloEspecular.fx", "VS_Main", "vs_4_0", &vsBuffer);
		...
		compileResult = CompileD3DShader(L"ModeloEspecular.fx", "PS_Main", "ps_4_0", &psBuffer);
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, especular, 0, 0, &especularMap, 0);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, D3DXVECTOR3 Direccion, D3DXVECTOR3 ColorLuz, float FA)
	{
		...
		d3dContext->PSSetShaderResources(2, 1, &especularMap);
